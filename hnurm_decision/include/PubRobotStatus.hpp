...
// 数据结构定义：
#include "decision_data_structure_def.hpp"
...
namespace hnurm_behavior_trees2{
    ...
    public:
        ...
        GlobalPose current_pose_{0.0, 0.0};

        // goal_pose_override，或者说当前导航目标点（goal_pose_callback）（原来的goal_pose）
        GlobalPose goal_override_pose{0.0, 0.0};

        // 裁判系统返回状态 血量、游戏进度、机器人欧拉角
        RobotRefereeStatus referee_status_{0.0, 0.0, {0.0f, 0.0f, 0.0f}};

        // 敌人距离自己现在位值的目标信息 目标状态、目标距离（原来的senddata）
        TargetData target_data{false, 0.0};
        // 目标在map上的位姿（包含坐标和四元数）
    ...
    private:
        ...
        // 检查机器人是否到达目标点
        bool goal_checker(GlobalPose cruise_goal, GlobalPose current_pose, double goal_tolerance);
        // 更新目标点信息
        void goal_updater(GlobalPose goal);

        // 系统初始化函数，加载机器人配置参数
        void initializaion();

        // 函数：calculate_distance 计算当前位置与目标位置之间的距离的平方
        double calculate_distance(GlobalPose current_pose, GlobalPose target_pose);

        // 函数：calculateAverage 计算多边形点集的平均坐标，作为机器人当前位置的估计值
        ...
        GlobalPose getCenter(const std::vector<hnurm_interfaces::msg::ZoneEndPoint2D> &end_points);

        // 函数：visualizeClosestArea 可视化离机器人最近的特殊区域，绘制特殊区域的路径线，发布标记到RViz可视化
        void visualizeClosestArea();

        // 函数：isinSpecialArea 判断机器人是否在特殊区域内
        bool isinSpecialArea(GlobalPose current_pose, std::string area);
        /*****************************************************特殊区域 end***************************************************/
        ...
        geometry_msgs::msg::PoseStamped update_goal(GlobalPose cruise_goal);

        // 函数：find_Away_point 计算远离目标的安全位置
        GlobalPose find_Away_point(float yaw, float distance);
        ...
        std::vector<GlobalPose> cruise_goals_; // 存储巡航模式下的目标点序列。在cruise_模式下，机器人会按顺序访问这些目标点
        std::vector<GlobalPose> buff_points_;  // 存储Buff点（补给点）的坐标集合
        GlobalPose enermy_outpost_;            // 敌人前哨战点坐标
        GlobalPose attack_outpost_point_;      // 攻击前哨战点坐标
        /*****************************************************存储系统 end*******************************************************/
